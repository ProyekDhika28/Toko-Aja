...
document.getElementById('input-metode').addEventListener('change', (e) => {
  ...
  document.getElementById('cash-kembalian-box').classList.toggle('hidden', e.target.value !== 'Cash');
  if (e.target.value === 'Cash') updateKembalian();
});
  ...
  container.innerHTML = list
    .map(b => {
      const infoPack = b.harga_pack
        ? ` • Pack ${formatRupiah(b.harga_pack)}${b.isi_per_pack ? ` (isi ${b.isi_per_pack} pcs)` : ''}`
        : '';
      return `<div class="card flex justify-between items-center">
      <div>
        <p class="font-semibold">${b.nama_barang}</p>
        <p class="text-xs text-slate-400">Modal ${formatRupiah(b.harga_modal)} • Jual ${formatRupiah(b.harga_jual)} • Stok ${b.stok}${infoPack}</p>
